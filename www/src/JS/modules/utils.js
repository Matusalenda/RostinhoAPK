...
export function switchView(direction) {
  if (direction === "next") {
    ...
    view2.displayName.innerText = `OPERADOR: ${appState.operatorName.slice(
      0,
      20,
    )}`;
  ...
  setTimeout(() => {
    ...
    console.log(
      `Estado da aplicação:\n ${JSON.stringify(
        appState.actualView === 0 ? "Tela 1" : "Tela 2",
      )}\n Operador: ${appState.operatorName}\n Modo: ${
        appState.isAuto ? "AUTO" : "MANUAL"
      }`,
    );
  }, 1000);
}
...
export function toggleAutoButton() {
  ...
  if (window.debug) {
    console.clear();
    console.log(`Modo ${appState.isAuto ? "AUTO" : "MANUAL"} ativado!`);
  }
}
...
export function scanPn() {
  ...
  if (appState.isAuto && appState.actualView === 1) {
    ...
    if (appState.lastPn === "" || appState.lastPn === currentPn) {
      ...
      if (window.debug) {
        console.clear();
        console.log(
          `Part-number lido: ${pnFormat(appState.lastPn)}\n Quantidade: ${
            appState.qtyCount
          }`,
        );
      }
